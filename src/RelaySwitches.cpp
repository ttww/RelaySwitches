/*
    ...
    Relay Board:
   https://www.electrodragon.com/w/ESP_Relay_Board_Hardware#Quick_Start_Guide
*/
...
https://github.com/jeroenvermeulen/JeVe_EasyOTA/blob/master/JeVe_EasyOTA.h
*/
#ifdef ESP8266
...
#define MAX_RELAYS_V1 8 // Maximum relays saved in EEPROM in version 1

#define EEPROM_SIZE_USED 64 // currently 20 in EEPROM_VERSION 1, but we want to have space for later...
typedef struct {
	uint32_t eepromSignature;
	uint16_t eepromVersion;
	boolean currentStates[MAX_RELAYS_V1];
	uint32_t switchCounts[MAX_RELAYS_V1];
} EEPROMSettings_V1;

//#if (RELAYS_COUNT > MAX_RELAYS_V1)
//#error "To much relays"
//#endif
...
{
	EEPROM.begin(EEPROM_SIZE_USED);
	EEPROM.get(0, eepromSettings);

	if (eepromSettings.eepromSignature != EEPROM_SIGNATURE) {
		eepromSettings.eepromSignature = EEPROM_SIGNATURE;
		eepromSettings.eepromVersion = EEPROM_VERSION;

		for (int i = 0; i < MAX_RELAYS_V1; i++) {
			eepromSettings.switchCounts[i] = 0;
			eepromSettings.currentStates[i] = 0;
		}
		eepromNeedsWrite = true;
	}
	if (eepromSettings.eepromVersion != EEPROM_VERSION) {
		// todo if needed...
		//eepromNeedsWrite = true;
	}
...
static void writeEEPROMifNeeded()
{
	if (eepromNeedsWrite) {
		mqttPublish("log/msg", "write eeprom");
		...
		EEPROM.commit();
		EEPROM.end();
		eepromNeedsWrite = false;
		mqttPublish("log/msg", "write eeprom done");
	}
}
...
#ifdef INVERT_RELAY
	digitalWrite(Relays[r], !onOffState);
#else
	digitalWrite(Relays[r], onOffState);
#endif

	eepromSettings.currentStates[r] = onOffState;
	eepromSettings.switchCounts[r]++;
...
static void initGpio()
{
	if (RELAYS_COUNT > 0) {
		for (int i = 0; i < RELAYS_COUNT; i++) {
			...
			if (eepromWasRead)
				switchRelay(i, eepromSettings.currentStates[i]); // restore last state
			else
				switchRelay(i, LOW);
		}
	}
...
static void publishRelay(int r)
{
	...
	mqttPublish("count/" + String(r, 10), String(eepromSettings.switchCounts[r]));
}
...
void setup()
{
	...
	readEEPROM();
	Serial.println("Read EEPROM DONE");
...
*/
	...
	while (WiFi.waitForConnectResult(25000) != WL_CONNECTED) {
...
void loop()
{
	...
	if (millis() - lastEEPROM > EEPROM_WRITE_INTERVAL) {
		writeEEPROMifNeeded();
		lastEEPROM = millis();
	}
